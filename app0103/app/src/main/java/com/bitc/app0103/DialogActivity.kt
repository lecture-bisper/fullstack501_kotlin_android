        ...
        toast.addCallback(
          object: Toast.Callback() {
            override fun onToastHidden() {
              super.onToastHidden()
              Log.d("csy-toast", "토스트 메시지 숨겨질 때 동작하는 이벤트")
            }
...
//      날짜 및 시간 정보를 가지고 있는 Calendar 객체를 가져옴, 싱글톤 방식으로 구현되어 있음
      ...
      val datePickerDialog = DatePickerDialog(this, object: DatePickerDialog.OnDateSetListener {
        override fun onDateSet(view: DatePicker?, year: Int, month: Int, dayOfMonth: Int) {
          Log.d("csy-datePicker", "년도 : $year, 달 : $month, 일 : $dayOfMonth")
...
//      }, calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH))
    ...
    binding.editTextTime.setOnClickListener {
      ...
      val timePickerDialog = TimePickerDialog(this, TimePickerDialog.OnTimeSetListener { view, hour, min ->
        Log.d("csy-timePicker", "설정된 시간 : ${hour}시 ${min}분")
...
//        AlertDialog 창 이외의 다른 부분을 클릭을 했을 경우 처리 방식 설정, 기본값 true, false 사용 시 지정한 버튼을 클릭해야만 AlertDialog 창이 닫힘
        setCancelable(false)
        show()
      }
...
//      alertDialog.setMessage("테스트 본문입니다. 맘에 드시나요??")
...
//      선택한 문자열을 저장하기 위한 리스트 객체
      ...
      AlertDialog.Builder(this).run {
        setIcon(android.R.drawable.ic_dialog_info)
        setTitle("Alert 다이얼로그에 체크 박스 출력")
//        체크 박스 출력, 이벤트 리스너를 람다식으로 사용
        setMultiChoiceItems(items, booleanArrayOf(false, false, false, false), {
//          which : 사용자가 선택한 체크박스의 index 번호
//          flag : 사용자가 선택한 체크박스 선택 여부 값, true/false
          dialog, which, flag ->
          Log.d("csy-alertDialog", "${items[which]}이 ${if (flag) "선택되었습니다" else "선택 해제 되었습니다"}")
          if (flag) {
...
//        AlertDialog에 라디오버튼을 추가하는 메소드
        setSingleChoiceItems(items, 1) {
          dialog, which ->
          Log.d("csy-alertDialog", "${items[which]} 를 선택했습니다.")
          selectItem = items[which]
        }
        setPositiveButton("닫기", {
            dialog, which ->
          Log.d("csy-alertDialog", selectItem)
