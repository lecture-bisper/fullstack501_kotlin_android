...
//  자바에서 구현 시
  ...
  var data3 = "hello"
  when (data3) {
    "hello" -> { println("data3 is hello")}
    "world" -> println("data3 is world")
    else -> println("data3 is not valid data3")
  }

//  is : 데이터 타입 확인 연산자, 지정한 데이터 타입이 맞으면 true 아니면 false 를 출력
//  in 시작..종료 : 범위 지정 연산자, 시작부터 종료까지의 데이터에 포함되어 있으면 true, 아니면 false 를 출력
  var data4: Any = 10
  when (data4) {
...
//  when 사용 시 조건 데이터를 생략해도 상관없음 (조건 데이터 생략 시 when 의 코드 블럭 안에서 조건식을 직접 모두 입력해야 함)
  var data5 = 10
  ...
  println("\n ----- 반복문 사용 ----- \n")
  println("----- for문 -----")
//  in : 지정한 시작 숫자부터 지정한 종료 숫자까지 1씩 증가하며 반복, <= 기호 사용
// until : 지정한 시작 숫자부터 지정한 종료 숫자까지 1씩 증가며서 반복, 종료 숫자는 포함하지 않음, < 기호 사용
