...
// 자식 클래스 Child11은 매개변수가 있고, 소스가 없는 생성자 보유
// 상속받는 부모 클래스의 기본 생성자를 입력하여 상속
class Child11(val childField2: Int): Parent1() {
  var childField1 = 1000

  fun childMethod() {
    println("자식 클래스 Child11의 멤버 메소드")
  }
}

// 생성자에 매개변수가 있고 소스코드는 없는 부모 클래스
...
// 부모 클래스 Parent2를 상속받은 자식 클래스 Child21
...
// 부모 클래스 Parent2를 상속받은 자식 클래스 Child22
...
// 부모 클래스 Parent3 를 상속받는 자식 클래스 Child3
class Child3: Parent3() {
//  부모 클래스에서 상속받은 멤버와 동일한 이름의 멤버 변수 및 메소드를 선언하여 오류가 발생함
//  오류 없이 사용하고자할 경우 오버라이딩이 필요함
...
// 접근제한자 : 코틀린에서는 private, protected, internal, public 4가지는 제공함
// 기존 자바와 동일하며, internal은 자바의 default와 동일함
// protected 는 클래스에서는 자바와 동일하지만 코틀린에서는 파일의 최상위 멤버로 변수와 함수가 존재하기 때문에 변수와 함수에는 protected를 사용할 수 없음
// 코틀린의 기본 접근제한자는 public임, 접근제한자 생략 시 public으로 동작함
open class Parent4 {
...
class Child4: Parent4() {
  fun child4Fun() {
//    접근 제한자가 public이므로 어디서나 사용 가능
    publicData++
//    접근 제한자가 protected이므로 Parent4 클래스를 상속받은 Child4 내부에서 사용 가능
    protectedData++
//    접근 제한자가 private이므로 자식 클래스인 Child4에서 사용할 수 없음
//    privateData++
  }
}
...
//  접근제한자가 public이므로 어디서든 사용 가능
  child4.publicData++
//  접근제한자가 protected 이므로 Parent4 클래스를 상속받은 Child4 클래스 내부에서는 사용 가능하나 외부에서는 사용 불가
//  child4.protectedData++
//  접근제한자가 private이므로 부모 클래스인 Parent4 내부에서만 사용 가능
//  child4.privateData++
  ...
  println("nd1 : ${nd1.name}, ${nd1.email}, ${nd1.age}")
  println("nd2 : ${nd2.name}, ${nd2.email}, ${nd2.age}")
...
//  일반 클래스로 객체를 생성 시 동일한 데이터가 들어가 있다고 하여도 메모리 상에 생성되는 위치가 다르기 때문에 메모리 주소가 달라서 서로 다른 데이터로 인식
  ...
  println("dc1 : ${dc1.name}, ${dc1.email}, ${dc1.age}")
  println("dc2 : ${dc2.name}, ${dc2.email}, ${dc2.age}")

//  데이터 클래스로 만들어진 객체를 서로 비교 시 객체의 주소를 비교하는 것이 아니라 서로가 가지고 있는 데이터를 비교하여 같은지 아닌지를 확인함
  result = dc1 == dc2
  println("dc1과 dc2 비교 : $result")
