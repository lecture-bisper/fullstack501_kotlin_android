...
//  xml 파일을 파싱한 ViewHolder를 여러개 가지는 Adapter 클래스
...
//  ViewHolder가 생성될 때 실행됨, 전체 UI을 생성함
...
//  생성된 리사이클러 뷰의 ViewHolder에 데이터를 연동
//  첫번째 매개변수로 RecyclerView.ViewHolder 타입의 객체를 가져옴, RecyclerView.ViewHolder 의 객체 안에 들어있는 것은 자식 클래스 타입의 객체인 MyViewHolder 가 들어있음
//  두번재 매개변수는 MyAdapter 클래스 타입의 객체를 생성 시 받아온 데이터의 index 번호
