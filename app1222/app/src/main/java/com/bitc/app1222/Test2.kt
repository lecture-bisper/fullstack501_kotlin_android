...
// kotlin에서 String 타입은 Java와 동일하게 사용 가능
...
//  Kotlin에서 Char 타입은 Java와 달리 정수 타입이 아니므로 정수와 비교 시 오류 발생
//  println(a8 == 65)

//  자바와 동일하게 사용된 String 타입
  var str1 = "Hello \n World!!"
//  kotlin에서 추가된 삼중따옴표 방식, 입력한 형태대로 그대로 사용 가능
...
//  문자열 템플릿 사용 시
  println("변수 출력 : $title, 함수 호출 : ${sum(10, 20)}, 덧셈 : ${10 + 20}")
//  기존 방식 사용 시
  println("변수 출력 : " + title + ", 함수 호출 : " + sum(10, 20) + ", 덧셈 : " + (10 + 20))
...
//  데이터 타입 Nothing으로 지정하여 null 만 저장함
  var data4: Nothing? = null
//  null 이외의 데이터 타입을 저장하기 때문에 오류 발생
//  data4 = 100

//  널 허용/ 널 불허용 : kotlin의 변수는 모든 변수가 객체이기 때문에 null을 저장할 수 있는 가능성이 존재함
...
// 반환 타입을 Unit로 지정하여 반환값이 없음을 표시
fun some1(): Unit {
  println(10 + 20)
}

// 반환 타입이 없을 경우 Unit를 생략해도 상관없음, 컴파일러가 자동으로 붙임
fun some2() {
  println(10 + 20)
}
