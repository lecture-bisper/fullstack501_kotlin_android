...
//      안드로이드 앱 안에 지정된 주소에 있는 미디어 파일 재생
    ...
    binding.btnUserMediaAlarmStop.setOnClickListener {
      Toast.makeText(this, "사용자 미디어를 알람 중지", Toast.LENGTH_SHORT).show()
      player.stop()
    }
...
//      사용자가 지정한 시간 및 진동 강도로 진동 알림 울리기
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
...
//    getActivity()를 통해서 지정한 앱의 화면으로 전환 요청
    var eventPendingIntent = PendingIntent.getActivity(this, 30, mainIntent, PendingIntent.FLAG_MUTABLE)
...
//    val dialogPendingIntent = PendingIntent.getActivity(this, 40, dialogIntent, PendingIntent.FLAG_MUTABLE)
//    builder.addAction(
//      NotificationCompat.Action.Builder(
